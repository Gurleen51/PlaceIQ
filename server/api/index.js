...
/* ═══════════════════════════════════════════
   ...
   Vercel has no persistent disk, so we store
   resume metadata only (no file saved to disk)
═══════════════════════════════════════════ */
const upload = multer({
...
/* ═══════════════════════════════════════════
   DATABASE — connection cached across
...
/* ═══════════════════════════════════════════
   ...
   (runs before every request in serverless)
═══════════════════════════════════════════ */
...
function getRequiredSkills(title) {
  ...
  if (t.includes("ml") || t.includes("machine learning")) return ["python","ml","tensorflow","pandas"];
...
async function generateMatches(userId) {
  ...
  matches.sort((a, b) => {
    if (b.is_preferred_role !== a.is_preferred_role) return b.is_preferred_role ? 1 : -1;
...
app.post("/auth/register", async (req, res) => {
  try {
    const { name, email, password, skills, education, experience, preferred_role, certifications, projects, cgpa } = req.body;
...
app.post("/auth/login", async (req, res) => {
  try {
    ...
    if (user.role === "student") {
      generateMatches(user.user_id).catch(err => console.error("Background match error:", err.message));
    }
    res.json({
      token,
      user: { name: user.name, role: user.role, user_id: user.user_id, skills: user.skills || "", preferred_role: user.preferred_role || "" },
    });
  } catch (err) {
...
app.get("/matches", async (req, res) => {
  try {
    ...
    res.json(matches);
  } catch { res.status(500).json({ error: "Failed to fetch matches." }); }
});
...
app.get("/skills-data", async (req, res) => {
  try {
    ...
    const skillMap = {};
    users.forEach(u => normalizeSkills(u.skills).forEach(sk => { skillMap[sk] = (skillMap[sk] || 0) + 1; }));
    res.json(Object.entries(skillMap).map(([skill, count]) => ({ skill, count })).sort((a,b) => b.count - a.count));
...
app.get("/salary-data", async (req, res) => {
  ...
  } catch { res.status(500).json({ error: "Failed to fetch salary data." }); }
});
...
app.put("/update-profile", auth, async (req, res) => {
  try {
    const { name, skills, education, experience, preferred_role, certifications, projects, cgpa } = req.body;
    const updated = await User.findOneAndUpdate(
      { user_id: req.user.user_id },
      { $set: { name, skills, education, experience, preferred_role, certifications: certifications || "", projects: projects || "", cgpa: cgpa != null && cgpa !== "" ? Number(cgpa) : null } },
      { new: true }
    );
...
app.get("/my-matches", auth, async (req, res) => {
  try {
    ...
    const matches = await db.collection("user_job_matches")
      .find({ user_id: req.user.user_id }).sort({ recommendation_rank: 1 }).toArray();
    res.json(matches);
  } catch { res.status(500).json({ error: "Failed to fetch matches." }); }
});
...
app.get("/my-dashboard-matches", auth, async (req, res) => {
  try {
    const db = mongoose.connection.db;
    let matches = await db.collection("user_job_matches")
      .find({ user_id: req.user.user_id, is_preferred_role: true })
      .sort({ recommendation_rank: 1 }).toArray();
    if (matches.length === 0) {
      await generateMatches(req.user.user_id);
      matches = await db.collection("user_job_matches")
        .find({ user_id: req.user.user_id, is_preferred_role: true })
        .sort({ recommendation_rank: 1 }).toArray();
    }
    res.json(matches);
  } catch (err) { res.status(500).json({ error: "Failed to fetch dashboard matches." }); }
});
...
app.get("/my-report", auth, async (req, res) => {
  try {
    ...
    const avgScore = matches.length ? Math.round(matches.reduce((s,m) => s + (m.overall_match_pct||0), 0) / matches.length) : 0;
    const gapFreq = {};
    matches.forEach(m => (m.missing_mandatory_skills||"").split(",").map(s=>s.trim()).filter(Boolean).forEach(sk => { gapFreq[sk] = (gapFreq[sk]||0)+1; }));
    const topGaps = Object.entries(gapFreq).sort((a,b) => b[1]-a[1]).slice(0,5).map(([skill,count]) => ({skill,count}));
    res.json({
      profile: { name:user.name, email:user.email, education:user.education||"Not specified", experience:user.experience||"Not specified", preferred_role:user.preferred_role||"Not specified", skills:user.skills||"", certifications:user.certifications||"", projects:user.projects||"", cgpa:user.cgpa, createdAt:user.createdAt },
      analytics: { total_matches:matches.length, eligible_jobs:eligible, avg_match_score:avgScore, top_skill_gaps:topGaps },
      top_matches: matches.slice(0,5).map(m => ({ company_name:m.company_name, job_title:m.job_title, location:m.location, salary:m.salary, overall_match_pct:m.overall_match_pct, eligible:m.eligible, matched_skills:m.matched_skills })),
...
app.post("/add-job", auth, async (req, res) => {
  try {
    ...
    if (!company_name || !job_title) return res.status(400).json({ error: "Company and title required." });
    const db = mongoose.connection.db;
    await db.collection("job_postings").insertOne({ company_name, job_title, location, offered_salary_lpa: Number(offered_salary_lpa)||0, openings: Number(openings)||0, createdAt: new Date() });
    res.status(201).json({ message: "Job added." });
  } catch { res.status(500).json({ error: "Failed to add job." }); }
});
...
app.get("/create-admin", async (req, res) => {
  try {
    if (await User.findOne({ email: "admin@placeiq.com" })) return res.json({ message: "Admin already exists." });
...
app.post("/admin/create-admin", auth, async (req, res) => {
  try {
    ...
    if (!requester || requester.role !== "admin") return res.status(403).json({ error: "Access denied." });
    ...
    res.status(201).json({ message: `Admin '${name}' created.`, user_id: admin.user_id });
...
app.delete("/admin/delete-admin/:id", auth, async (req, res) => {
  try {
    ...
    if (target.user_id === req.user.user_id) return res.status(400).json({ error: "Cannot delete yourself." });
    await User.findByIdAndDelete(req.params.id);
